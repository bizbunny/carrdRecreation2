...
//This looks like it's to help make the page work in different browsers in the first section
(
    function() {
        ...
        var getVar = function(name) {
            ...
            {
                b = a[k].split('=');if (b[0] == name)return b[1];
            }
            return null;
        };//var getVar = function(name)
        var errors = {
            handle: function(handler) {window.onerror = function(message, url, line, column, error) {
        ...
        if (client.os == 'android') {
            ...
            if (client.osVersion <= 11)(function() {
                ...
                rule = sheet.cssRules[0];
                rule.style.cssText = 'height: calc(100% + 60px)';on('focus', function(event) {
                    $body.classList.add('ios-focus-fix');}, true);on('blur', function(event) {
                        $body.classList.remove('ios-focus-fix');}, true);})();$body.classList.add('is-touch');
        }//else if (client.os == 'ios') 
                ...
                (function() {
                    ...
                    if (a.length > 0) {
                        ...
                        if (r.style.width.match('calc')) {
                            r.style.opacity = 0.9999;setTimeout(function() {
                                r.style.opacity = 1;
                            }, 100);
                        }//if (r.style.width.match('calc'))  
                        else {
        ...
        if (!client.canUse('object-fit')) {
            (function() {
                ...
                for (i=0; i < xx.length; i++) {
                    ...
                    c.style['backgroundPosition'] = x.dataset.position;c.style['backgroundRepeat'] = 'no-repeat';
                    ...
                    if (x.classList.contains('full')&& (x.parentNode && x.parentNode.classList.contains('full'))&& (x.parentNode.parentNode && x.parentNode.parentNode.parentNode && x.parentNode.parentNode.parentNode.classList.contains('container'))&& x.parentNode.children.length == 1) {
                        (function(x, w) {
                            var p = x.parentNode.parentNode,f = function() {
                                ...
                                t = setTimeout(function() {
                                    if (getComputedStyle(p).flexDirection == 'row') {
                                        if (w)w.style['height'] = '100%';x.style['height'] = (p.scrollHeight + 1) + 'px';
                                    } 
                                    else {
                            ...
                            },t;//var p = x.parentNode.parentNode,f = function() 
                            on('resize', f);
                            on('load', f);
                            (f)();
                        })(x, w);//(function(x, w)
